...
<div class="row g-4">
    ...
    <div class="list-group list-group-flush">@forelse($section->quizzes as $quiz)<div class="list-group-item px-0"><div class="d-flex justify-content-between"><div><div class="fw-semibold">{{ $quiz->title }}</div><div class="small text-secondary">{{ $quiz->duration_minutes ? $quiz->duration_minutes.' menit' : 'Tanpa batas durasi' }} · {{ $quiz->attempt_limit }} percobaan</div></div><span class="badge bg-azure-lt text-azure">{{ ucfirst($quiz->status) }}</span></div></div>@empty<div class="text-secondary">Belum ada quiz aktif.</div>@endforelse</div>
    </section>
</div>
</div>
</div>
